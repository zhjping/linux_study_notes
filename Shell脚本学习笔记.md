...
- __"$*"型内置变量__
    ...
    "$@"    传递给脚本或函数的所有参数，被双引号""包含时，与$*稍有不同。
            不被双引号“”包含时，都以$1 $2 …$n的形式输出所有参数。当它们被双
            引号“”包含时，“$*”会将所有的参数作为一个整体，以“$1 $2 … $n”的形式
...
- __for循环__
    ...
    for循环里的in列表(list)是可选的，如果省略，shell循环会遍历整个
    命令行参数。就好像你已经输入了for i in "$@"
    ```
...
- __${CC:-gcc}__
    ...
    ```(text)
    CC=${CC:-gcc}
    如果$CC存在并且不为空那么CC=$CC，否则就是CC=gcc
    :-为三元运算。
    ```
